	...
	public Main() {
		...
		button_signin.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				Homepage nextHomepage = new Homepage();
				if (login.login(input_txtemail.getText(), input_txtpass_hide.getText())) {

					setVisible(false);
					nextHomepage.setVisible(true);
					nextHomepage.setEmail(input_txtemail.getText());
				}
				if (!login.login(input_txtemail.getText(), input_txtpass_hide.getText())) {
